	...
	public class Class1Tests
	{
		...
		public void Class1_ShouldReturn_TwoServiceMockValues()
		{
			...
			using (var serviceLocator = new Class1ServiceLocator())
			{
				...
				values = class1.GetServiceValues()?.ToList();
			}

			// assert
			values.Should().NotBeNull();
			values.Count.Should().Be(2);
			values[0].Should().Be(Class1ServiceLocator.Service1Value);
			values[1].Should().Be(Class1ServiceLocator.Service2Value);
		}

		[Fact]
		public void Class1_ShouldReturn_TwoServiceMockValues_ByTestableServiceLocator()
		{
			...
			IList<string> values;
			using (var serviceLocator = new Class1ServiceLocator(container))
			{
				serviceLocator.Lock();
